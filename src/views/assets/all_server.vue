<template>
    <div class="mytab">
        ...
        <div class="mytab-header">
            <el-form :inline="true" :model="config">
                ...
                <el-popover
                        ...
                        trigger="click">
                    <el-form-item>
                        <el-checkbox v-model="config.showMachine">宿主机</el-checkbox>
                        <el-checkbox v-model="config.showCreateTime">创建时间</el-checkbox>
                        <el-checkbox v-model="config.showKefu">使用者</el-checkbox>
                        <el-checkbox v-model="config.showOps">管理者</el-checkbox>
                    </el-form-item>
        ...
        <el-table border class="commonTable"
            ...
            <el-table-column prop="host_machine" label="宿主机" v-if="config.showMachine"></el-table-column>
  ...
  export default {
    ...
    methods: {
      getAssetsTableData(event, my_url) {
        if (!my_url) {
          my_url = `api/v1/assets/info/?page=${this.config.page}&limit=${this.config.page_size}`;
        }
      ...
      searchAssets() {
        let url = `api/v1/assets/info/?${this.config.select}=${this.config.search}`;
      ...
      exportExcel() {
        ...
        try {
          FileSaver.saveAs(new Blob([wbout], {type: 'application/octet-stream'}), '资产信息表.xlsx')
        } catch (e) {
